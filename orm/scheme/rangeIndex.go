...
func (index *RangeIndex) validate(columnsSet map[string]bool) {
	...
	for _, ik := range index.IK {
		utils.Validatef(columnsSet[ik], "table %s: range index %s: ik %s is not in columns", index.Table.Name, index.Name, ik)
	...
	for _, column := range index.Columns {
		utils.Validatef(columnsSet[column], "table %s: range index %s: column %s is not in columns", index.Table.Name, index.Name, column)
